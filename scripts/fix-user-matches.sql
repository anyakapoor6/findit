-- Fix for get_user_matches function to show matches for both sides, including user IDs
-- This ensures that both the lost and found listing owners see the match and can distinguish which side is theirs
...
BEGIN
  ...
  SELECT 
    ...
    l2.user_id as matched_listing_user_id,
    m.score,
    m.match_reasons
  FROM matches m
  ...
  WHERE l1.user_id = get_user_matches.user_id
     OR l2.user_id = get_user_matches.user_id
